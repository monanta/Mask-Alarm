...
window = tkinter.Tk()
window.title("Alarm Masker")
window.wm_iconbitmap('logobps.ico')
...
def mulai():
    ...
    btn_tangkapan.pack(anchor=tkinter.CENTER, expand=True)
    keluarkansuara(pred)
    delay = 15  
    window.mainloop()
...
def tangkapan():
    subprocess.call("explorer .\\tertangkap", shell=True)
